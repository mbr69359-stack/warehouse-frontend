...
<script>
import { getInOrders, confirmInOrder } from '../../api/inOrder'
export default {
  data() { return { list: [], total: 0, loading: false, query: { current: 1, size: 10, status: null } } },
  created() { this.loadData() },
  methods: {
    async loadData() { this.loading = true; const r = await getInOrders(this.query).finally(() => { this.loading = false }); this.list = r.data.records; this.total = r.data.total },
    async handleConfirm(id) {
      await this.$confirm('确认入库后库存将增加，是否继续？', '提示', { type: 'warning' })
      await confirmInOrder(id); this.$message.success('入库确认成功'); this.loadData()
    }
  }
}
</script>
