...
export default {
  ...
  methods: {
    ...
    render() {
      ...
      const inMap  = Object.fromEntries(this.inData.map(d => [d.date, Number(d.count)]))
      const outMap = Object.fromEntries(this.outData.map(d => [d.date, Number(d.count)]))
      this.chart.setOption({
        tooltip: { trigger: 'axis' },
        legend: { data: ['入库单', '出库单'], bottom: 0 },
